...
function saveNotes(authHeader: () => string, notes: string) {
  return axios
    .put(
      "http://localhost:8080/clock-notes",
      { notes }, // request body
      { headers: { Authorization: authHeader() } } // request config
    )
    .then((response: AxiosResponse) => response.data);
            ...
            <>
              ...
              <Button
                ...
                onClick={() =>
                  apiWrapper(
                    () =>
                      saveNotes(authHeader, notes).then(() =>
                        setOldNotes(notes)
                      ),
                    setError,
                    setIsNotesLoading
              ...
              <Button
                ...
                text={
                  (notes == oldNotes ? "" : "Save Notes and ") + "Clock Out"
                }
                color="green"
                onClick={() =>
                  apiWrapper(
                    () =>
                      clockOut(authHeader, notes).then(
                        (data: {
                          is_clocked_in: boolean;
                          clock_in_time: string;
                        }) => {
